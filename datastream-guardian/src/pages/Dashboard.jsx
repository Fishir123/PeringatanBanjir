...
const Dashboard = () => {
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {['safe', 'alert', 'warning', 'danger'].map(s => (<div key={s} className={`rounded-lg px-4 py-3 text-center font-semibold text-sm ${statusColorMap[s]} ${data.status === s ? 'ring-2 ring-offset-2 ring-offset-background' : 'opacity-50'}`} style={{ ['--tw-ring-color']: undefined }}>
            {statusLabels[s]}
          </div>))}
      </div>
